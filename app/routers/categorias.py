...
def get_categoria(id: int, db = Depends(database.get_db))-> Categoria:
    cursor = db.cursor()
...
def actualizar_categoria(id: int, actualizarCategoria: ActualizarCategoria, db=Depends(database.get_db))-> Categoria:
    cursor = db.cursor()
    cursor.execute("SELECT * FROM categorias WHERE id = ?", (id,))
    resultado = cursor.fetchone()
    if resultado is None:
    ...
    cursor.execute("UPDATE categorias SET titulo = ? WHERE id = ?", (actualizarCategoria.titulo, id))
    db.commit()
    return Categoria(id=id, nombre=actualizarCategoria.titulo)

@router.delete("/categorias{id}")
def eliminar_categoria(id: int, db=Depends(database.get_db))-> Categoria:
    cursor = db.cursor()
    cursor.execute("SELECT * FROM categorias WHERE id = ? ", (id,))
    resultado = cursor.fetchone()
    if resultado is None:
